...
SPEAKERS = {"F02": "Low", "F03": "Very Low", "F04": "Mid", "F05": "High", \
   "M01": "Very Low", "M04": "Very Low", "M05": "Mid", "M07": "Low", "M08": "High", \
       "M09": "High", "M10": "High", "M11": "Mid", "M12": "Very Low", "M14": "High", "M16": "Low"}

INTELLIGIBILITY = {"Very Low": 0, "Low": 1, "Mid": 2, "High": 3}
...
def main():
    ...
    wer_speaker_total = [0]*15
    wer_speaker_incorrect = [0]*15
    ...
    for clip_path in df["wav_filename"]:
        ...
        # intelligibility and speaker indices
        level = SPEAKERS[speaker]
        idx_level = INTELLIGIBILITY[level]
        idx_speaker = list(SPEAKERS.keys()).index(speaker)
        ...
        # total
        wer_incorrect[4] += edits
        wer_total[4] += length

        # intelligibility
        wer_incorrect[idx_level] += edits
        wer_total[idx_level] += length
        ...
        # total
        cer_incorrect[4] += edits
        cer_total[4] += len(y_true)
        
        # intelligibilty
        cer_incorrect[idx_level] += edits
        cer_total[idx_level] += len(y_true)
    ...
    for i, level in enumerate(INTELLIGIBILITY.keys()):
        print(f"{level} WER: {round(wer_incorrect[i]/wer_total[i]*100, 2)} %")
        print(f"{level} CER: {round(cer_incorrect[i]/cer_total[i]*100, 2)} % \n")
    print(f"Total word error rate: {round(wer_incorrect[4]/wer_total[4]*100, 2)} %")
    print(f"Total character error rate: {round(cer_incorrect[4]/cer_total[4]*100, 2)} %")
    
    # intelligibility results
    wer_row = []
    cer_row = []
    for i, level in enumerate(INTELLIGIBILITY.keys()):
        wer_row.append(round(wer_incorrect[i]/wer_total[i]*100, 2))
        cer_row.append(round(cer_incorrect[i]/cer_total[i]*100, 2))
    wer_row.append(round(wer_incorrect[4]/wer_total[4]*100, 2))
    cer_row.append(round(cer_incorrect[4]/cer_total[4]*100, 2))
    
    # create data frame for intelligibility results
    df = pandas.DataFrame([wer_row, cer_row], columns = ["Very Low", "Low", "Mid", "High", "Total"], index = ["WER", "CER"])
    ...
    df = pandas.DataFrame([speaker_row], columns = list(SPEAKERS.keys()))
